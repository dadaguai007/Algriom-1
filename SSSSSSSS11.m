...
% 文件存储路径
datapath='Output\BTB_SIC_best_alpha_40k';
...
%     Dc=mean(ReceivedSignal);
    ...
    % 去掉直流
    re_signal=re_signal-mean(re_signal);
    ...
    % 算法迭代
%     alpha=0.02;
    alpha= 0.1283; % 最佳alpha
    for j=1:1
        [recoverI,error]=iteraElimate(re_signal+Dc,pd_input,fs,alpha,Dc,Vdither(1));
        % 对信号进行切分，并提出全部信号
...
%         [ReceivedSignal,dc]=Receiver.Preprocessed_signal(totalPortion1);
        ...
        pd_input=recoverI;
        re_signal=ReceivedSignal-mean(ReceivedSignal);
    end
...
LengendArrary=["btb Total ",'group','btb w SIC'];
